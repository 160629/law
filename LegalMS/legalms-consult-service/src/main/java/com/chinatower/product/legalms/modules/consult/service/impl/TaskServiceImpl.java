package com.chinatower.product.legalms.modules.consult.service.impl;
...
public class TaskServiceImpl implements TaskService {
    ...
    private final String[] flowType = {"", "引诉纠纷", "", "纠纷处理", "案件交办", "案件协办", "法律文书办理", "证照申请", "授权申请"};
...
//    TaskMapper taskMapper;
    ...
    public ProcessResult queryPersonWorkItems(TaskVO taskVO) {
        ...
        try {
            ...
            return new ProcessResult(ProcessResult.ERROR, CoreConstClass.OPERATION_FAILED);
        }

        IWFWorklistQueryManager queryManager = client.getWorklistQueryManager();
        loginManager.setCurrentUser(userId, username, CoreConstClass.TENANLID, null);
        // 设置查询相关条件
        // 工作项条件
        ...
        switch (chooseType) {
            case "1":
                try {
                    wfWorkItems = queryManager.queryPersonWorkItemsWithBizInfo(userId, "ALL", "ALL", CoreConstClass.FLOW_BUSINESS_TABLENAME, wiSql.toString(), bizSql.toString(), windList, bizBindList, pageCond);
                    break;
                } catch (WFServiceException e) {
                    log.error(CoreConstClass.TASK1, e);
                    log.error(ProcessResult.ERROR,e);
                    return new ProcessResult(ProcessResult.ERROR, "查询代办列表失败");
                }
            case "2":
                try {
                    setBizBindListOfCurwork(bizSql, bizBindList, "审批中", "已退回");
                    wfWorkItems = queryManager.queryPersonFinishedWorkItemsWithBizInfo(userId, "ALL", false, CoreConstClass.FLOW_BUSINESS_TABLENAME, wiSql.toString(), bizSql.toString(), windList, bizBindList, pageCond);
                    break;
                } catch (WFServiceException e) {
                    log.error(CoreConstClass.TASK2, e);
                    log.error(ProcessResult.ERROR,e);
            ...
            case "3":
                try {
                    ...
                    log.error(CoreConstClass.TASK3, e);
                    log.error(ProcessResult.ERROR,e);
    ...
    private void setBizBindListOfCurwork(StringBuilder bizSql, List bizBindList, String field1, String field2) {
...
//        }
//    }
    private void setWindList(String processInstId, String workItemInstId, StringBuilder wiSql, List windList) {
    ...
    private void setWindList(String startTime, String endTime, String time, SimpleDateFormat simpleDateFormat, StringBuilder wiSql, List windList) {
        if (StringUtils.isNotBlank(startTime)) {
            ...
            try {
                ...
                log.error(CoreConstClass.DATE_FORMAT_FAIL, e);
                throw new MyOwnRuntimeException(CoreConstClass.DATE_FORMAT_FAIL);
            }
        }
...
//                    task3 = queryPersonWorkItems(json);
                    ...
                    taskNumVO.setId(StringUtil.getId())
                            ...
                            .setToDoTask((int) ((Map<String, Object>) task1.getData()).get(CoreConstClass.TASK_COUNT) + (int) ((Map<String, Object>) task2.getData()).get("count"))
                            .setFinishTask((int) ((Map<String, Object>) task2.getData()).get(CoreConstClass.TASK_COUNT))
                            .setEndTask((int) ((Map<String, Object>) task3.getData()).get(CoreConstClass.TASK_COUNT));
